...
├── domain/
│   ├── account/
│   │   ├── Account.java                 ← 엔터티 (aggregate root)
│   │   ├── AccountId.java
...
- **HTTP 컨트랙트 (내부)**:
  - [specs/contracts/http/internal/auth-to-account.md](../../contracts/http/internal/) (credential lookup 요청 수신)
...
- **이벤트 발행**: [specs/contracts/events/account-events.md](../../contracts/events/) — `account.created`, `account.status.changed`, `account.locked`, `account.unlocked`, `account.deleted`. 모두 outbox 경유
- **퍼시스턴스**: MySQL — `accounts`, `profiles`, `account_status_history`, `outbox_events`
- **Redis**: 가입 이메일 검증 코드 TTL, 중복 가입 요청 dedupe
...
**필수 시나리오**: 중복 이메일 가입 409 / 상태 기계의 불허 전이 400 / 삭제 요청 후 유예 기간 중 복구 가능 / 유예 만료 후 PII 익명화 / 모든 상태 변경이 `account_status_history`에 row로 기록.
