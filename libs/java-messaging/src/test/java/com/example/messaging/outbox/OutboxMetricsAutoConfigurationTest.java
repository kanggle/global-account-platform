...
class OutboxMetricsAutoConfigurationTest {
    ...
    // ── conditional back-off tests (ApplicationContextRunner) ──────────────
        ...
        contextRunner
                ...
                .run(ctx -> {
                    ...
                    assertThat(ctx.getBean(OutboxFailureHandler.class))
                            .isSameAs(ctx.getBean(UserHandlerConfig.class).userHandler());
                });
    }
    ...
    // ── metric name / counter increment tests (direct invocation) ──────────
    ...
    static class UserHandlerConfig {
        ...
        OutboxFailureHandler userHandler() {
            return (eventType, aggregateId, e) -> { /* no-op sentinel */ };
        }
    }
}
