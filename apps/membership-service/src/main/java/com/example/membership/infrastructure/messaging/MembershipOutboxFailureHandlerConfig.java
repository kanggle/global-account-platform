 ...
 * {@code membership_outbox_publish_failures} Micrometer counter (tagged with
 ...
 */
@Configuration
class MembershipOutboxFailureHandlerConfig {

    @Bean
    OutboxFailureHandler outboxFailureHandler(MeterRegistry meterRegistry) {
        return (eventType, aggregateId, e) ->
                meterRegistry.counter("membership_outbox_publish_failures", "event_type", eventType).increment();
    }
}
