...
>
> **남은 3건 (residual)** — 모두 Testcontainers 인프라 안정성 재검토 필요, 본 task 가 아닌
> 후속 task 로 분리 권장:
>
> - `DlqRoutingIntegrationTest` — Kafka ErrorHandlingDeserializer DLQ 라우팅 실측
> - `DetectionE2EIntegrationTest` — detection 파이프라인 윈도우/집계 실측
> - `OAuthLoginIntegrationTest` — MySQL testcontainer 가 테스트 중 dies (#18 CI 실측).
>   dynamic port 개선은 적용됨. Testcontainers reuse / healthcheck / resource 재설정 필요.
