...
>
> **2026-04-19 residual pass**: 남은 3건 중 **2건에서 코드 레벨 근본 원인 발견 및 수정**,
> 1건은 tightened residual 로 유지.
>
> - `DetectionE2EIntegrationTest` — ✅ **복원**. 근본 원인: 테스트가 accountId 를
>   `"acc-e2e-velocity-" + UUID.randomUUID()` (총 53 chars) 로 생성했는데
>   `suspicious_events.account_id` 는 VARCHAR(36). MySQL 은 insert 시 data
>   truncation 으로 거부, 롤백된 결과 `findByAccountId...` 가 빈 리스트를 반환해
>   AssertionError (row not found) 로 수렴. 이전에는 detection 파이프라인 버그로
>   오독됐음. accountId 를 bare UUID (36 chars) 로 축소해 복원.
> - `DlqRoutingIntegrationTest` — ✅ **복원**. 근본 원인:
>   `src/test/resources/application.yml` 이 `spring.kafka.consumer.*-deserializer`
>   를 plain `StringDeserializer` 로 덮어쓰고 있어서, DLQ 경로가 의존하는
>   `ErrorHandlingDeserializer` + `StrictJsonStringDeserializer` 체인이 테스트
>   컨텍스트에 로드되지 않음. invalid-bytes 시나리오는 `DeserializationException`
>   을 던질 deserializer 가 없어서 DLQ 로 가지 않음. `@DynamicPropertySource` 에서
>   consumer deserializer 를 prod 와 동일하게 재설정 (test-scope only) 해 복원.
> - `OAuthLoginIntegrationTest` — ❌ **residual 유지, 주석 tighten**. 추가 코드
>   레벨 조사 결과 `OAuthLoginUseCase#callback` 이 `@Transactional` 안에서 OAuth
>   provider + account-service 2개 외부 HTTP 호출을 수행하는 구조가 드러남. JDBC
>   커넥션이 양쪽 round-trip 동안 pinned 되어 CI 부하 상황에서 Hikari 3s
>   connection-timeout 과 중첩돼 MySQL container 레벨 "connection refused" 로
>   surface 되는 시나리오가 유력함. 수정안은 (a) HTTP 호출을 트랜잭션 경계 밖으로
>   분리 — 프로덕션 동작 변경이라 별도 task (b) 테스트 프로파일 Hikari 튜닝 +
>   container resource 재조정. 본 task 범위 밖이므로 residual 로 유지하고 주석에
>   구체적 follow-up 지침 기록.
...
- [ ] `OAuthLoginIntegrationTest`의 `@Disabled` 제거 + CI에서 7 tests all passed — **residual (follow-up task)**: `OAuthLoginUseCase#callback` 의 `@Transactional` 외부 HTTP 호출 분리 또는 테스트 Hikari 튜닝 필요
- [x] `AuthIntegrationTest.refreshTokenReuseDetected`의 `@Disabled` 제거 ✅ (2026-04-19)
- [x] `AccountSignupIntegrationTest.signup_duplicateEmail_returns409`의 `@Disabled` 제거 ✅ (2026-04-19)
- [x] `ActivateSubscriptionIntegrationTest`의 `@Disabled` 제거 ✅ (2026-04-19)
- [x] `SubscriptionExpirySchedulerTest`의 `@Disabled` 제거 ✅ (2026-04-19)
- [x] `DetectionE2EIntegrationTest`의 `@Disabled` 제거 ✅ (2026-04-19: accountId 길이 36자로 축소)
- [x] `DlqRoutingIntegrationTest`의 `@Disabled` 제거 ✅ (2026-04-19: consumer deserializer prod 와 일치하도록 `@DynamicPropertySource` 재설정)
- [x] `LoginHistoryImmutabilityTest`의 `@Disabled` 제거 ✅ (2026-04-19: event_id VARCHAR(36) 준수)
- [x] AuthIntegrationTest.java의 loose assertion(`TOKEN_REUSE_DETECTED | SESSION_REVOKED`)을 단일 값으로 정밀화 ✅ (2026-04-19)
- [ ] `./gradlew :apps:auth-service:test` 로컬(Docker 동작 환경)에서 통과 — OAuth residual 때문에 pending
- [ ] `./gradlew build` CI에서 backend job 전체 green — OAuth residual 때문에 pending
